  ...
  return (
    <div className="flex-1 min-w-0" ref={ref}>
      ...
      <div
        ...
        className={`relative p-3 sm:p-4 pb-8 sm:pb-10 rounded-xl border cursor-pointer transition-all duration-200 ${
          open
      ...
      >
        <div className="flex flex-col lg:flex-row lg:items-center lg:gap-2 mb-2 sm:mb-3">
          <div className={`w-8 sm:w-10 h-8 sm:h-10 rounded-lg ${theme==='dark' ? m.darkIconBg : m.iconBg} flex items-center justify-center`}>
            <CategoryIcon category={skill} className={theme==='dark' ? m.darkIconColor : m.iconColor} />
          </div>

          <div className="flex items-center justify-between lg:flex-1 mt-1 lg:mt-0">
            <h4 className={`text-sm font-semibold ${theme==='dark' ? 'text-white' : 'text-gray-900'} capitalize`}>{skill}</h4>
            <span className={`text-xs ${theme==='dark' ? 'text-gray-500' : 'text-gray-400'}`}>{selectedInCategory.length}</span>
          </div>
        </div>
        ...
        <div className={`absolute bottom-2 sm:bottom-3 right-2 sm:right-3 w-5 sm:w-6 h-5 sm:h-6 rounded-full flex items-center justify-center transition-colors ${
          open ? "bg-purple-100" : `${theme==='dark' ? 'bg-gray-700' : 'bg-gray-100'}`
        }`}>
          <svg className={`w-3 sm:w-3.5 h-3 sm:h-3.5 transition-transform duration-200 ${open ? "rotate-180 text-purple-600" : `${theme==='dark' ? 'text-gray-500' : 'text-gray-400'}`}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
      ...
      {open && (
        ...
        <div className={`fixed inset-0 m-auto w-[90vw] sm:w-[70vw] md:w-[60vw] max-h-[60vh] overflow-y-auto ${theme==='dark' ? 'bg-gray-900/90 border-gray-700' : 'bg-white/15 border-white/30'} backdrop-blur-2xl rounded-2xl shadow-2xl border p-3 sm:p-5 z-50 popIn font-['Inter']`}>
          <div className={`flex justify-between items-center mb-4 pb-3 border-b ${theme==='dark' ? 'border-gray-700' : 'border-white/20'}`}>
            <h3 className={`text-xl font-semibold capitalize ${theme==='dark' ? 'text-white' : 'text-gray-800'} tracking-wide`}>{skill}</h3>
            <div className="flex items-center gap-2">
              <span className={`text-sm font-medium px-3 py-1 rounded-full ${theme==='dark' ? 'bg-gray-800 text-gray-300' : 'bg-white/40 text-gray-700'}`}>{selectedInCategory.length}</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-4 gap-1.5 sm:gap-2 overflow-hidden">
            {filteredSkills.map((item:Skill)=>{
              ...
              return(
                ...
                className={`flex items-center gap-1.5 sm:gap-2 px-2 sm:px-3 py-1.5 sm:py-2.5 rounded-xl text-xs sm:text-sm font-medium transition-all duration-100 cursor-pointer min-w-0 ${
                  isSelected
